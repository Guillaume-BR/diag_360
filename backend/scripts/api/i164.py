#!/usr/bin/env python3
"""
Indicateur i164 : taux de population précaireses par EPCI
Source : Atlas Santé
URL : https://sirse.atlasante.fr/#c=indicator&i=pop_active_sal.tx_emploiprec&s=2022&view=map3
"""
...
# Configuration
URL = "https://sirse.atlasante.fr/#c=indicator&i=pop_active_sal.tx_emploiprec&s=2022&view=map3"
DEFAULT_INDICATOR_ID = "i164"
DEFAULT_YEAR = 2024
DEFAULT_SOURCE = "i164.csv"
...
def traitement_precaire() -> pd.DataFrame:
    ...
    logger.info("Téléchargement des données du taux de population précaire")

    data_precaire = pd.read_csv(path_file, sep=",")

    # Traitement de la table des populations précaires
    df_precaire = data_precaire.rename(
        columns={
            "Code": "code_insee",
            "Libellé": "nom_com",
            "Part de la population en emploi précaire 2022": "valeur",
        }
    )
...
def clean_and_prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calcul par epci du taux de population précaire
    query = """ 
    SELECT 
...
def run(indicator_id: str) -> None:
    ...
    try:
        ...
        df_precaire = traitement_precaire()
        df_processed = clean_and_prepare_df(df_precaire)
